...
- 远程配置：支持国内 JSON 配置通道和 Firebase Remote Config 兜底，用于广告位、弹窗通知和版本更新策略。
...
- Cloudflare R2 JSON remote config
...
构建带国内远程配置地址的 Android release 包：
...
```text
config/shenghui-config.json
```

GitHub Actions 会校验 JSON 并发布到 R2。配置说明见：

```text
docs/firebase-remote-config.md
```

远程配置只应保存公开运营信息，例如广告位、弹窗通知、版本更新策略。不要把 API Key、签名密钥、用户隐私数据写入远程配置文件。
