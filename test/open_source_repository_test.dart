...
void main() {
  ...
  test('repository keeps local product data out of source', () {
    ...
    final removedConfigExample = ['shenghui', 'product', 'config'].join('-');
    final removedBuildDoc = ['official', 'build', 'config'].join('-');
    expect(
      File('config/$removedConfigExample.example.json').existsSync(),
      isFalse,
    );
